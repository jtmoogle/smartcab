...
## Date: Aug/Sept, 2017
## 
## This file /smartcab/simulation.py was mainly for smart cab simulation
## 
##  ** All functions and codes were not changed.  
##  - I only added option while learning
##      to  save q-table: state, action, reward into a csv file, 
##      so, I can use these table for question 7, 8
## 
###########################################
...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=True, log_metrics=False, optimized=False):
        ...
        print " Simulation.init( size={}, update_delay={}, display={}, log_metrics={}, optimized={} ) ".format(
                size, update_delay, display, log_metrics, optimized )
        ...
        if self.log_metrics:
            ...
            if a.learning:
                if self.optimized: # Whether the user is optimizing the parameters and decay functions
                    ...
                    self.table2_filename = os.path.join("logs","sim_improved-qlearntbl.csv")
                    self.table3_filename = os.path.join("logs","sim_improved-qlearntbl.2.csv")
                else: 
                    ...
                    self.table2_filename = os.path.join("logs","sim_default-qlearntbl.csv")
                    self.table3_filename = os.path.join("logs","sim_default-qlearntbl.2.csv")
    ...
    def run(self, tolerance=0.05, n_test=0):
        ...
        print " Simulation.run( tolerance= {}, n_test={} ) ".format(tolerance, n_test)    
        self.quit = False
        ...
        if self.log_metrics:
            ...
            if a.learning:                
                ...
                #----- write to table 2: q table: state action reward
                self.table2_fields = ['state1', 'state2', 'state3', 'state4', 'action1','reward1', 'action2', 'reward2', 'action3', 'reward3', 'action4', 'reward4']
                self.table2_file = open(self.table2_filename, 'wb')
                self.table2_writer = csv.DictWriter(self.table2_file, fieldnames=self.table2_fields)
                self.table2_writer.writeheader()
                f2 = self.table2_file
                #----- write to table 3: q table: state action reward
                self.table3_fields = ['state1', 'state2', 'state3', 'state4', 'action','reward']
                self.table3_file = open(self.table3_filename, 'wb')
                self.table3_writer = csv.DictWriter(self.table3_file, fieldnames=self.table3_fields)
                self.table3_writer.writeheader()
                f3 = self.table3_file
                #-----

                for state in a.Q:
                    f.write("{}\n".format(state))
                    val = re.sub( '[()\']', '', str(state))                    
                    f2.write( val )  # -- f2 --
                    for action, reward in a.Q[state].iteritems():
                        f.write(" -- {} : {:.2f}\n".format(action, reward))
                        f2.write( ",{},{:.2f}".format(action, reward))  # -- f2 --
                        f3.write( "{},{},{:.2f}\n".format( val, action, reward)) # -- f3 --
                    f.write("\n")  
                    f2.write("\n")
                f.close()
                f2.close()
                f3.close()
                #elf.table_file.close()
                #elf.table2_file.close()
                #elf.table3_file.close()
